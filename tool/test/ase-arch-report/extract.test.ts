...
test("extract: private method on class is excluded", async () => {
    const tmpDir = path.join(import.meta.dirname, "fixtures", "java-visibility")
    const file = path.join(tmpDir, "Mixed.java")
    const parser = new Parser(WASM_DIR)
    const tree = await parser.parse(file, "java")
    const grammar = await parser.getGrammar("java")
    const symbols = await extractSymbols(tree, grammar, "java", file, QUERIES)
    const cls = symbols.find((s) => s.name === "Mixed")
    assert.ok(cls !== undefined)
    assert.ok(cls!.members.find((m) => m.name === "publicApi") !== undefined, "publicApi must be present")
    assert.ok(cls!.members.find((m) => m.name === "protectedHelper") !== undefined, "protectedHelper must be present")
    assert.equal(cls!.members.find((m) => m.name === "privateInternal"), undefined, "privateInternal must be excluded")
    assert.equal(cls!.members.find((m) => m.name === "packagePrivate"), undefined, "packagePrivate must be excluded")
})

test("extract: interface methods without modifier are kept (interfaces are implicitly public)", async () => {
    const tmpDir = path.join(import.meta.dirname, "fixtures", "java-visibility")
    const file = path.join(tmpDir, "Service.java")
    const parser = new Parser(WASM_DIR)
    const tree = await parser.parse(file, "java")
    const grammar = await parser.getGrammar("java")
    const symbols = await extractSymbols(tree, grammar, "java", file, QUERIES)
    const iface = symbols.find((s) => s.name === "Service")
    assert.ok(iface !== undefined && iface!.kind === "interface")
    assert.ok(iface!.members.find((m) => m.name === "doThing") !== undefined)
})
